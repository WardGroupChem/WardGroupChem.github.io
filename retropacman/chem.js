...
var Chem = (function () {
  ...
  function parse(smiles) {
    var atoms = [], bonds = [];
    var stack = [], prev = -1, pend = null, rings = {}, hlNext = false;
    var s = smiles, i = 0;

    function addBond(a, b, sym, hl) {
    ...
    while (i < s.length) {
      ...
      // '^' (not standard SMILES — local extension): highlight the NEXT bond
      // as the retrosynthetic disconnection (drawn dashed red).
      if (c === '^') { hlNext = true; i++; continue; }
      if ('-=#:/\\'.indexOf(c) >= 0) { pend = c; i++; continue; }
      // '{MgBr|BrMg}' (local extension): an abbreviation label drawn as text,
      // bonded like an atom. Second form is used when the bond arrives from
      // the right (e.g. Ph3P=, BrMg–, O2N–). Digits are drawn as subscripts.
      if (c === '{') {
        var close = s.indexOf('}', i);
        var parts = s.slice(i + 1, close).split('|');
        atoms.push({ el: '*', abbr: parts[0], abbrRev: parts[1] || parts[0], arom: false, bracket: true, hExp: 0, charge: 0 });
        var ai = atoms.length - 1;
        if (prev >= 0) addBond(prev, ai, pend, hlNext);
        pend = null; hlNext = false; prev = ai;
        i = close + 1;
        continue;
      }
      ...
      if (/[0-9%]/.test(c)) {
        ...
        if (rings[num]) {
          ...
          addBond(r.atom, prev, sym, hlNext || r.hl);
          delete rings[num];
        } else {
          rings[num] = { atom: prev, bond: pend, hl: hlNext };
        }
        pend = null; hlNext = false;
        continue;
      }
      ...
      if (prev >= 0) addBond(prev, idx, pend, hlNext);
      pend = null; hlNext = false;
      prev = idx;
    }
  ...
  function needsLabel(mol, idx) {
    var a = mol.atoms[idx];
    if (a.el !== 'C' || a.charge !== 0 || mol.adj[idx].length === 0) return true;
    // two-atom molecules (CH3I, CH3OH, CH3NH2, CH3MgBr): write the methyl
    // out as H3C / CH3 rather than leaving a bare stick
    if (mol.atoms.length === 2 && mol.adj[idx].length === 1) return mol.atoms[mol.adj[idx][0].nb].el !== 'C';
    return false;
  }

  // width of a label string with digits drawn as subscripts
  function abbrWidth(str, F) {
    var w = 0;
    for (var i = 0; i < str.length; i++) w += /[0-9]/.test(str[i]) ? textWidth(str[i], F * 0.75) : textWidth(str[i], F);
    return w;
  }
  // SVG text body with digits as subscripts (dy shifts down, then back up)
  function subText(str, F) {
    var out = '', down = false, d = F * 0.28;
    for (var i = 0; i < str.length; i++) {
      var ch = str[i], dig = /[0-9]/.test(ch);
      if (dig && !down) { out += '<tspan dy="' + f(d) + '" font-size="' + f(F * 0.75) + '">'; down = true; }
      else if (!dig && down) { out += '</tspan><tspan dy="' + f(-d) + '">'; down = false; }
      out += ch;
    }
    if (down) out += '</tspan>';
    return out;
  ...
  function abbrGeometry(mol, idx) {
    var a = mol.atoms[idx], P = mol.pos[idx], F = CFG.font;
    var sumDx = 0;
    mol.adj[idx].forEach(function (e) { sumDx += mol.pos[e.nb].x - P.x; });
    var rev = sumDx > 1; // bond arrives from the right -> write e.g. BrMg, Ph3P
    var text = rev ? a.abbrRev : a.abbr;
    // the attached element symbol: leading (forward) or trailing (reversed)
    var m = rev ? /([A-Z][a-z]?)[0-9]*$/.exec(text) : /^([A-Z][a-z]?)/.exec(text);
    var anchorW = textWidth(m ? m[1] : text[0], F);
    var totalW = abbrWidth(text, F);
    var left = rev ? P.x + anchorW / 2 - totalW : P.x - anchorW / 2;
    return {
      abbr: true, text: text, rev: rev, anchorW: anchorW, h: /[0-9]/.test(text) ? 2 : 0,
      hw: anchorW / 2 + 1.5, hh: F * 0.46,
      left: left, right: left + totalW
    };
  }

  function labelGeometry(mol, idx) {
    var a = mol.atoms[idx], P = mol.pos[idx], F = CFG.font;
    if (a.el === '*') return abbrGeometry(mol, idx);
    var symW = textWidth(a.el, F);
    var hStr = a.h ? 'H' + (a.h > 1 ? a.h : '') : '';
    var hW = a.h ? textWidth('H', F) + (a.h > 1 ? textWidth(String(a.h), F * 0.75) : 0) : 0;
    var sumDx = 0, sumDy = 0, maxAbsDx = 0;
    mol.adj[idx].forEach(function (e) {
      var ddx = mol.pos[e.nb].x - P.x;
      sumDx += ddx; sumDy += mol.pos[e.nb].y - P.y;
      if (Math.abs(ddx) > maxAbsDx) maxAbsDx = Math.abs(ddx);
    });
    var hLeft = sumDx > 1; // bonds come from the right -> write HO, H2N
    // atom inside a chain (e.g. the NH of an amide) whose bonds leave to both
    // sides: put the H in the free space above/below, not on top of a bond
    var hVert = 0;
    if (a.h && mol.adj[idx].length >= 2 && Math.abs(sumDx) < CFG.bond * 0.6 && maxAbsDx > CFG.bond * 0.3) {
      hVert = sumDy < 0 ? 1 : -1; // 1 = below, -1 = above
      hLeft = false;
    }
    var side = hVert ? 0 : hW;
    return {
      symW: symW, hW: hW, hLeft: hLeft, hVert: hVert, h: a.h,
      hw: symW / 2 + 1.5, hh: F * 0.46,
      left: P.x - Math.max(symW, hVert ? hW : 0) / 2 - (hLeft ? side : 0),
      right: P.x + Math.max(symW, hVert ? hW : 0) / 2 + (hLeft ? 0 : side) + (a.charge ? F * 0.5 : 0)
    };
  }
  ...
  function drawBonds(mol, labels) {
    var pos = mol.pos, L = CFG.bond, normal = '', hl = '';
    mol.bonds.forEach(function (bd) {
      var s = drawOne(bd);
      if (bd.hl) hl += s; else normal += s;
    });
    return { normal: normal, hl: hl };
    ...
    function drawOne(bd) {
      ...
      if (bd.order === 1) { out += line(x1, y1, x2, y2); return out; }
      if (bd.order === 3) {
  ...
  function drawLabels(mol, labels) {
    ...
    labels.forEach(function (g, idx) {
      ...
      var base = P.y + F * 0.36;
      if (g.abbr) {
        out += '<text x="' + f(g.left) + '" y="' + f(base) + '" text-anchor="start">' + subText(g.text, F) + '</text>';
        return;
      }
      out += '<text x="' + f(P.x) + '" y="' + f(base) + '" text-anchor="middle">' + a.el + '</text>';
      if (g.h) {
        var hTxt = 'H' + (g.h > 1 ? '<tspan dy="' + f(F * 0.28) + '" font-size="' + f(sub) + '">' + g.h + '</tspan>' : '');
        if (g.hVert) out += '<text x="' + f(P.x) + '" y="' + f(base + g.hVert * F * 1.05) + '" text-anchor="middle">' + hTxt + '</text>';
        else if (g.hLeft) out += '<text x="' + f(P.x - g.symW / 2) + '" y="' + f(base) + '" text-anchor="end">' + hTxt + '</text>';
  ...
  function prepare(smiles) {
    ...
    mol.pos.forEach(function (p, idx) {
      ...
      var t = g ? p.y - g.hh - (mol.atoms[idx].charge ? 3 : 0) - (g.hVert === -1 ? CFG.font * 1.05 : 0) : p.y;
      var b = g ? p.y + g.hh + (g.h > 1 ? 3 : 0) + (g.hVert === 1 ? CFG.font * 1.05 : 0) : p.y;
  ...
  function toSVG(p, opts) {
    ...
    return '<svg xmlns="http://www.w3.org/2000/svg" class="mol" role="img"' +
      ...
      (function () {
        var b = drawBonds(p.mol, p.labels);
        return '<g fill="none" stroke="currentColor" stroke-width="' + CFG.lineWidth + '" stroke-linecap="round">' + b.normal + '</g>' +
          (b.hl ? '<g class="cut-bond" fill="none" stroke-width="' + (CFG.lineWidth + 0.6) + '" stroke-linecap="butt" stroke-dasharray="4 3" style="stroke:var(--cut, #FF5C74)">' + b.hl + '</g>' : '');
      })() +
